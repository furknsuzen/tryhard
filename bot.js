...
client.channels.get('796062817421754400').join()
})
...
//client.on("message", async message => {
  ...
  //  if (message.deletable) message.delete();
   // message.channel.send(`Hey ${message.author}, linki yöneticime gönderdim. Eğer sorunlu bir linkse banını bekleyebilirsin.`)
//})

client.on("message", async message => {
  if(message.author.id === client.user.id) return;
  if(message.guild) return;
  client.channels.get('809891581259022377').send(new Discord.RichEmbed().setAuthor("Yeni Bir DM", client.user.avatarURL).setFooter(message.author.tag, message.author.avatarURL).setDescription(`**Gönderenin ID:** ${message.author.id}`).setTimestamp().addField("Mesaj", message.content).setColor("RANDOM"))
})
...
.setTitle(`RoskvaBots | Bilgilendirme`)
.setTimestamp()
.setColor("GREEN")
...
let add = client.channels.get("809891581259022377")
...
let remove = client.channels.get("809891581259022377")
...
client.on("message", async msg => {
    ...
    let i = await db.fetch(`reklamFiltre_${msg.guild.id}`) 
          if (i == 'acik') {
              const reklam = ["discord.app", "discord.gg","discordapp","discordgg",];
            if (reklam.some(word => msg.content.toLowerCase().includes(word))) {
                try {
                  if (!msg.member.hasPermission("MANAGE_GUILD")) {   
                    ...
                    let embed = new Discord.RichEmbed()
                    .setColor(0xffa300)
                    .setFooter('RoskvaBots  -|-  Reklam engellendi.', client.user.avatarURL)
                    .setAuthor(msg.guild.owner.user.username, msg.guild.owner.user.avatarURL)
                    .setDescription("RoskvaBots Reklam sistemi, " + `***${msg.guild.name}***` + " adlı sunucunuzda reklam yakaladım.")
                    .addField('Reklamı yapan kişi', 'Kullanıcı: '+ msg.author.tag +'\nID: '+ msg.author.id, true)
                    .addField('Engellenen mesaj', msg.content, true)
                    .setTimestamp()                   
                    msg.guild.owner.user.send(embed)                       
                    return msg.channel.send(`${msg.author.tag}, **Bu linki yöneticime bildirdim, eğer reklamsa banın kutlu olsun!**`).then(msg => msg.delete(25000));
                  }             
                } catch(err) {
                  console.log(err);
                }
...
if (!modlogkanal) return;
	if (channel.type === "text") {
				...
				.setFooter(`RoskvaBots | Mod-Log Sistemi Kanal ID: ${channel.id}`)
				modlogkanal.send({embed});
			};
			if (channel.type === "voice") {
				let embed = new Discord.RichEmbed()
    ...
    if (xp > xpToLvl) {
      ...
      if (kanal) {
        client.channels
          .get(kanal.id)
          .send(
            message.ping.member.user.username +
